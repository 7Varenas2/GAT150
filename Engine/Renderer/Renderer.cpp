...
namespace neum
{
	...
	{
		SDL_SetRenderDrawColor(m_renderer, 35, 240, 255, 255);
		SDL_RenderDrawLineF(m_renderer, (float)x1, (float)y1, (float)x2, (float)y2);
	}

	void Renderer::DrawLine(const neum::Vector2& v1, const neum::Vector2& v2, const neum::Color& color)
	{
	...
	void Renderer::DrawPoint(float x, float y)
	{
		...
		SDL_RenderDrawPoint(m_renderer, (float)x, (float)y);
	}

	void Renderer::DrawPoint(const neum::Vector2& v, const neum::Color& color)
	{
		SDL_SetRenderDrawColor(m_renderer, color.r, color.g, color.b, color.a);
